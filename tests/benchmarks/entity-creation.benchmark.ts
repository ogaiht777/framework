...
function benchmark(name: string, fn: () => void, iterations: number = 10000): void {
    const start = Date.now(); // Use Date.now() for timing
    for (let i = 0; i < iterations; i++) {
        fn();
    }
    const end = Date.now(); // Use Date.now() for timing
...
RegisterCommand('runbenchmarks', (source: number, args: string[], rawCommand: string) => {
